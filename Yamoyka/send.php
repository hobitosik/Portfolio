<?php
  ...
  $mail->Subject = 'Лендинг Ямойка / Записаться без очереди';
  ...
  if(!$mail->Send()) {
    ...
    $body=file_get_contents("http://sms.ru/sms/send?api_id=APIID&to=NUMBERPHONE&text=".urlencode("Записаться без очереди. Имя: " . $_POST['name']. ". Телефон: " . $_POST['phone'])); 
