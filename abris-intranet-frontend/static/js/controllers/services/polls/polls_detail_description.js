angular.module('myApp').controller('PollsDetailDescriptionCtrl',
function ($scope, $state, $http, $filter, $q, jwtHelper, poll, predicates) {
	$scope.poll = poll;
	$scope.replies_predicate = predicates.parse_replies_predicate(poll.replies);
	...
	function get_replies () {
		return $http.get(api + 'poll/' + $scope.poll.id + '/get_replies/').then(function (resp) {
			return resp["data"];
		}, function (resp) {
			console.log(resp);
			return null;
	...
	$scope.send_answer_to = function (index) {
		...
		$http.post(api + 'poll/' + $scope.poll.id + '/add_comment/', sendIt).then(function (resp) {
			...
			$scope.replies_predicate = predicates.parse_replies_predicate(++$scope.poll.replies);
