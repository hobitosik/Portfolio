angular.module('myApp').controller('BestUsersDetCtrl',
function ($scope, $state, $http, $filter, $q, jwtHelper, bestUser, vote, predicates, favorites) {
	...
	$scope.breadcrumb = {
		...
		"main.users.best": "Лучшие сотрудники"
	}

	var user = best.user.profile;
	$scope.data = best;
	$scope.data.isFavorite = favorites.isFavorite($scope.data, 'bestemployee');
	$scope.data.date_formatted = moment(best["date"], 'YYYY-MM-DD').format("DD.MM.YYYY");
	$scope.breadcrumb_active = user["first_name"] + " " + user["last_name"] + " " + user["middle_name"];
	$scope.replies_predicate = predicates.parse_replies_predicate(best.replies);
	...
	function get_replies () {
		return $http.get(api + 'bestemployee/' + $scope.data.id + '/get_replies/').then(function (resp) {
			return resp["data"];
		}, function (resp) {
			console.log(resp);
			return null;
	...
	$scope.send_answer_to = function (index) {
		...
		$http.post(api + 'bestemployee/' + $scope.data.id + '/add_comment/', sendIt).then(function (resp) {
